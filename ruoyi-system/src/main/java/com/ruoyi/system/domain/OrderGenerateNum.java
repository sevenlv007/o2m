...
public class OrderGenerateNum {
    // 使用单例模式，不允许直接创建实例
    private OrderGenerateNum() {}

    // 创建一个空实例对象，类需要用的时候才赋值
    private static OrderGenerateNum g = null;

    // 单例模式--懒汉模式
    public static synchronized OrderGenerateNum getInstance() {
        if (g == null) {
            g = new OrderGenerateNum();
        }
        return g;
    }
